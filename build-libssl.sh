...
run_configure()
{
  ...
  set +e
  if [ "${LOG_VERBOSE}" == "verbose" ]; then
    ./Configure ${LOCAL_CONFIG_OPTIONS} shared no-hw no-asm no-engine no-tests | tee "${LOG}"
  else
    (./Configure ${LOCAL_CONFIG_OPTIONS} shared no-hw no-asm no-engine no-tests > "${LOG}" 2>&1) & spinner
  fi
